...
| `js/jellyfin.js` | floating Jellyfin music player: collapsed spinning-vinyl-disc button → click expands panel (art, seek, prev/play/next, shuffle/repeat/volume, search, tracklist); streams **always as transcoded AAC 256k** via `Audio/{id}/stream`; server-side seeks via `StartTimeTicks` (with `fetchBlobAndSeek` fallback when `audio.seekable` is `0–0` despite finite `audio.duration` — live `fMP4` `empty_moov` case, where native `audio.currentTime` is stuck at 0 and the identical-file `startTimeTicks` transcode is ignored; Blob is fetched as `blob:` URL, `serverStart` reset to 0, `loadedmetadata` then `currentTime=pos`); `RunTimeTicks` fallback for duration when `audio.duration` is unavailable (transcode); Media Session with `seekto` (same Blob guard); auto-plays a random Chinese-title track once consent is granted (`warning:cleared` event, or `system_warning_consent` localStorage flag already `true` on load) — Chinese = title has Han characters and no kana (mixed CN+EN titles count; kana titles are Japanese and excluded); auto-collapses when expanded and the user scrolls `.content-area` (`#home`/`#blogs`/`#about`/post content), `#sidebar`, or `#mobile-nav-tray` (via capture-phase `scroll` listener with zone flag `__jfScrollCollapseZone` and `MutationObserver` for the dynamic tray; `window.jfRegisterScrollCollapseZone(el)` allows producers to register eagerly), but **not** when scrolling `.jf-tracklist` inside the player (excluded by `jf-player`/`jf-tracklist` ancestor check); viewport `window`/`document` scrolls also collapse as the content fallback; self-inits, hides itself if the proxy is unconfigured | `window.jellyfinPlayer`, `window.jfRegisterScrollCollapseZone`; `#jf-player`, `.jf-*` |
